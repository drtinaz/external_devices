...
class DbusSwitch(VeDbusService):
    def __init__(self, service_name, device_config, output_configs, serial_number, mqtt_client,
        ...
        self.section_name = device_config.name
        ...
        if not main_custom_name:
            parts = self.section_name.split('_')
            if len(parts) >= 3:
                module_idx = parts[1]
                switch_idx = parts[2]
        ...
        self.add_path('/CustomName', main_custom_name, writeable=True, onchangecallback=self.handle_dbus_change)
    ...
    def add_output(self, output_data):
        ...
        # Output CustomName - saved to 'output_customname' in config
        self.add_path(f'{settings_prefix}/CustomName', output_data['output_custom_name'], writeable=True, onchangecallback=self.handle_dbus_change)
    ...
    def handle_dbus_change(self, path, value):
        # Handle main service CustomName - saves to 'customname'
        if path == '/CustomName':
            self.save_config_change(self.section_name, 'customname', value)
            return True
        ...
        if "/SwitchableOutput/output_" in path:
            try:
                ...
                if "/State" in path:
                    ...
                    # Output CustomName saves to 'output_customname'
                    if key_name == 'CustomName':
                        self.save_config_change(self.section_name, 'output_customname', value)
                    else:
                        self.save_config_change(self.section_name, key_name, value)
                    return True
            except Exception as e:
...
class DbusDigitalInput(VeDbusService):
    ...
    def __init__(self, service_name, device_config, serial_number, mqtt_client, bus):
        ...
        initial_type_int = self.DIGITAL_INPUT_TYPES.get(initial_type_str, self.DIGITAL_INPUT_TYPES['disabled'])
        ...
        try:
            ...
            try:
                ...
                if isinstance(incoming_json, dict) and "value" in incoming_json:
                    ...
                    logger.warning(f"DbusTempSensor: JSON payload for topic '{topic}' does not contain 'value' key.")
                    return
            except json.JSONDecodeError:
                try:
...
class DbusTankSensor(VeDbusService):
    ...
    def __init__(self, service_name, device_config, serial_number, mqtt_client, bus):
        ...
        if is_valid_topic(raw_topic):
            ...
            logger.warning(f"Tank '{self['/CustomName']}': Neither RawValueStateTopic nor LevelStateTopic are valid.")
        ...
        try:
            ...
            try:
                ...
                if isinstance(incoming_json, dict) and "value" in incoming_json:
                    ...
                    logger.warning(f"DbusTankSensor: JSON payload for topic '{topic}' does not contain 'value' key.")
                    return
            except json.JSONDecodeError:
                try: 
                    value = float(payload_str)
        ...
        if section_lower.startswith('switch_') and re.match(r'^switch_\d+_\d+$', section_lower):
            try:
                ...
                if not serial_number:
                    ...
                    logger.warning(f"Serial number not found for [{section}]. Generating random serial: {serial_number}")
                
                device_bus = dbus.bus.BusConnection(dbus.Bus.TYPE_SYSTEM)
                
                service_name = f'com.victronenergy.switch.external_{serial_number}'
                
                # Get the output custom name from config
                output_custom_name = device_config.get('output_customname', device_config.get('CustomName', f'switch {module_idx}-{switch_idx}'))
            ...
            if device_class:
                try:
                    ...
                    active_services.append(service)
                    logger.debug(f"Successfully initialized D-Bus service for [{section}] of type '{device_type_string}'.")
